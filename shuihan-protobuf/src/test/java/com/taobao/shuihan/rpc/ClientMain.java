...
public class ClientMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if(!cmd.hasOption("d")) {
            ...
            System.out.println("Use run delay from cmd line: " + runDelay + "ms");
        }
        final long runTime;
        if(!cmd.hasOption("t")) {
                ...
                public void run() {
                    while(!stopped.get()) {
                        if(tag) {
                            ...
                            if(!person.equals(ret)) { // FIXME DO NOT check every time!
                                correct.set(false);
                                stopped.set(true);
                                mainThread.interrupt();
                            }
        ...
        if(outputFile == null) {
            if(correct.get()) {
                System.out.printf(format,
                            System.currentTimeMillis(), rpcFactory.getAuthorId(), correct,
                            1.0 * invokeTime / runTime * 1000, invokeTime, runTime);
            }
            else {
            ...
            if(correct.get()) {
                ...
                out.printf(format_wrong_result,
                        System.currentTimeMillis(), rpcFactory.getAuthorId(), correct);
            }
            out.flush();
        }
